...
public class AWSAppSyncClient {
    ...
    private var networkStatusWatchers: [NetworkConnectionNotification] = []
    private let appSyncConfiguration: AWSAppSyncClientConfiguration
    let httpTransport: AWSNetworkTransport
    ...
    private let appSyncMQTTClient: AppSyncMQTTClient = AppSyncMQTTClient()
    private let subscriptionsQueue = DispatchQueue(label: "SubscriptionsQueue", qos: .userInitiated)
    private let subscriptionMetadataCache: AWSSubscriptionMetaDataCache?
    private var accessState: ClientNetworkAccessState = .Offline
