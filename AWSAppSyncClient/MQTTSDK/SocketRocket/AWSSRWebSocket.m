...
// AWS IoT sends all binary frames so there's no need for text frames
// anyway; disable use of the ICU library code.
//
//#if TARGET_OS_IPHONE
//#define HAS_ICU
//#endif
...
@interface NSString (AWSSRWebSocket)
...
@interface NSURL (AWSSRWebSocket)
...
@implementation NSData (AWSSRWebSocket)
...
@implementation NSString (AWSSRWebSocket)
...
@interface AWSSRWebSocket ()  <NSStreamDelegate>

@property (nonatomic) AWSSRReadyState readyState;
...
- (void)setReadyState:(AWSSRReadyState)aReadyState;
{
...
{
    assert(_url);
    NSAssert(_readyState == AWSSR_CONNECTING, @"Cannot call -(void)open on SRWebSocket more than once");
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:AWSSRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:AWSSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:AWSSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_updateSecureStreamOptions;
{
    if (_secure) {
        ...
        if ([_urlRequest AWSSR_SSLPinnedCertificates].count) {
    ...
    switch (requestNetworkServiceType) {
        ...
        default:
            ...
            if (@available(iOS 12.0, *)) {
                if (requestNetworkServiceType == NSURLNetworkServiceTypeResponsiveData) {
...
- (void)close;
{
    [self closeWithCode:AWSSRStatusCodeNormal reason:nil];
}
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason;
{
    ...
    dispatch_async(_workQueue, ^{
        if (self.readyState == AWSSR_CLOSING || self.readyState == AWSSR_CLOSED) {
            return;
        }
        
        BOOL wasConnecting = self.readyState == AWSSR_CONNECTING;
...
- (void)send:(id)data;
{
    NSAssert(self.readyState != AWSSR_CONNECTING, @"Invalid State: Cannot call send: until connection is open");
...
{
    NSAssert(self.readyState == AWSSR_OPEN, @"Invalid State: Cannot call send: until connection is open");
    // TODO: maybe not copy this for performance
...
- (void)handleCloseWithData:(NSData *)data;
{
    ...
    if (dataSize == 1) {
        ...
        _closeCode = AWSSRStatusNoStatusReceived;
    }
    
    [self assertOnWorkQueue];
    
    if (self.readyState == AWSSR_OPEN) {
    ...
    switch (opcode) {
        case SROpCodeTextFrame: {
            ...
            if (str == nil && frameData) {
                [self closeWithCode:AWSSRStatusCodeInvalidUTF8 reason:@"Text frames must be valid UTF-8"];
                dispatch_async(_workQueue, ^{
                    [self closeConnection];
                });
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(AWSSRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(AWSSRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(AWSSRWebSocket *self, NSData *data) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:AWSSRWebSocketErrorDomain code:2145 userInfo:[NSDictionary dictionaryWithObject:@"Error writing to stream" forKey:NSLocalizedDescriptionKey]]];
             return;
        }
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!frame) {
        [self closeWithCode:AWSSRStatusCodeMessageTooBig reason:@"Message too big"];
        return;
    }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
        ...
        NSArray *sslCerts = [_urlRequest AWSSR_SSLPinnedCertificates];
        if (sslCerts) {
            ...
            if (!_pinnedCertFound) {
                dispatch_async(_workQueue, ^{
                    [self _failWithError:[NSError errorWithDomain:AWSSRWebSocketErrorDomain code:23556 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid server cert"] forKey:NSLocalizedDescriptionKey]]];
                });
                return;
            }
...
@implementation  NSURLRequest (AWSSRCertificateAdditions)

- (NSArray *)AWSSR_SSLPinnedCertificates;
{
    return [NSURLProtocol propertyForKey:@"AWSSR_SSLPinnedCertificates" inRequest:self];
}

@end

@implementation  NSMutableURLRequest (AWSSRCertificateAdditions)
...
- (void)setAWSSR_SSLPinnedCertificates:(NSArray *)AWSSR_SSLPinnedCertificates;
{
    [NSURLProtocol setProperty:AWSSR_SSLPinnedCertificates forKey:@"AWSSR_SSLPinnedCertificates" inRequest:self];
}
